...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
  ...
  <!-- Incoming requests -->
  <div class="card">
    <div class="card-header"><span class="card-title"><i data-lucide="inbox"></i> Solicitudes recibidas</span></div>
    ...
    <div style="display:flex;flex-direction:column;gap:12px">
      ...
      <div style="padding:16px;background:var(--surface2);border-radius:12px;border:1px solid <?= $s['status']==='pending' ? 'rgba(245,158,11,0.3)' : 'var(--border)' ?>">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
          ...
          <span style="color:var(--muted);font-size:0.8rem">quiere que hagas</span>
        </div>
        ...
        <div style="margin-bottom:8px">
          <span class="swap-badge"><i data-lucide="banknote" style="width:10px;height:10px"></i> Te ofrece €<?= number_format($s['compensation'], 2) ?></span>
        </div>
        <?php endif; ?>
        <?php if ($s['message']): ?>
        ...
        <div style="display:flex;gap:8px">
          <form method="post" action="<?= site_url('/chores/swap/' . $s['id'] . '/accept') ?>">
            <?= csrf_field() ?>
            <button class="btn btn-sm btn-primary"><i data-lucide="check" style="width:12px;height:12px"></i> Aceptar</button>
          </form>
          <form method="post" action="<?= site_url('/chores/swap/' . $s['id'] . '/decline') ?>">
            <?= csrf_field() ?>
            <button class="btn btn-sm btn-danger"><i data-lucide="x" style="width:12px;height:12px"></i> Rechazar</button>
          </form>
        </div>
        <?php else: ?>
        <span class="badge <?= $s['status']==='accepted' ? 'badge-done' : 'badge-missed' ?>">
          <?php if ($s['status']==='accepted'): ?>
            <i data-lucide="check" style="width:10px;height:10px"></i> Aceptado
          <?php else: ?>
            <i data-lucide="x" style="width:10px;height:10px"></i> Rechazado
          <?php endif; ?>
        </span>
        <?php endif; ?>
      </div>
  ...
  <!-- Outgoing requests -->
  <div class="card">
    <div class="card-header"><span class="card-title"><i data-lucide="send"></i> Solicitudes enviadas</span></div>
    ...
    <div style="display:flex;flex-direction:column;gap:12px">
      ...
      <div style="padding:16px;background:var(--surface2);border-radius:12px;border:1px solid var(--border)">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px">
          ...
          $statusText  = ['pending'=>'Esperando','accepted'=>'Aceptado','declined'=>'Rechazado'][$s['status']] ?? $s['status'];
          ?>
          <span class="badge <?= $statusClass ?>"><i data-lucide="<?= $statusIcon ?>" style="width:10px;height:10px"></i> <?= $statusText ?></span>
        </div>
        <div style="font-size:0.8rem;color:var(--muted)">
          Pedido a <strong><?= esc($s['target_name']) ?></strong> · <?= date('d/m/Y', strtotime($s['created_at'])) ?>
        </div>
        <?php if ($s['compensation'] > 0): ?>
        <div style="margin-top:6px"><span class="swap-badge"><i data-lucide="banknote" style="width:10px;height:10px"></i> Ofreciste €<?= number_format($s['compensation'], 2) ?></span></div>
        <?php endif; ?>
        <?php if ($s['status'] === 'pending'): ?>
